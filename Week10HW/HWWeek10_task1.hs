...
main = do
    ...
    -- my tests for idea functions
    -- print $ setLevel (getLevel numberBTree 2) 2
    -- print $ traverseSetBFS numberBTree
...
height Nil = 0
height tree = helper 0
 where
     helper k
      | nodesAtLvlK == [] = k
      | otherwise = helper (k + 1)
       where
           nodesAtLvlK = getLevel tree k
...
average tree = roundTwo $ ((sum $ dfsTraversed) / (fromIntegral (length $ dfsTraversed)))
 where
     dfsTraversed = traverseDFS tree

-- I think i didn't understand the task
sumLeaves :: (Num a) => BTree a -> a
sumLeaves tree = sum $ dfsTraversed
 where
     dfsTraversed = traverseDFS tree


-- Actually I didn's use '==' :DD(couldn't figure out another way)
areEqual :: (Eq a) => BTree a -> BTree a -> Bool
areEqual Nil Nil = True
areEqual tree1 tree2 = not $ (traverseDFS tree1) /= (traverseDFS tree2)



-- setLevel :: [a] -> Int -> [(Int, a)]
-- setLevel vs k = [(k, x) | x <- vs]

-- traverseSetBFS :: (Eq a) => BTree a -> [(Int, a)]
-- traverseSetBFS Nil = []
-- traverseSetBFS t = helper 0
--  where
--      helper k
--       | nodesAtLvlK == [] = []
--       | otherwise = setNodesAtLvlK ++ helper (k + 1)
--        where
--            nodesAtLvlK = getLevel t k
--            setNodesAtLvlK = setLevel nodesAtLvlK k
...
setLevels tree = helper 0 tree
 where
     helper _ Nil = Nil
     helper k (Node val left right) = Node (k, val) (helper (k + 1) left) (helper (k + 1) right)
