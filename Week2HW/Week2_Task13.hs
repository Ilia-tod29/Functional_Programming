...
findSum a b n
 ...
 | otherwise = genAndCalcSequence 1 0
  where
      genAndCalcSequence :: Int -> Int -> Int
      genAndCalcSequence power result
       | power == n + 1 = result
       | otherwise = genAndCalcSequence (power + 1) (result + a + (sequence1 0 0 b power))
        where
